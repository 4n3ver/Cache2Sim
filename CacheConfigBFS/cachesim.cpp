...
static uint64_t vc_access(uint64_t target, cache_stats_t *p_stats) {
    if (MAX_VC_SIZE) {
        ...
        for (std::deque<uint64_t>::iterator iter = vc.begin(); iter < vc.end();
             iter++) {
...
void complete_cache(cache_stats_t *p_stats) {
    ...
    double mr1 = (p_stats->read_misses_l1 + p_stats->write_misses_l1) /
                 ((double) p_stats->accesses);
    double mr2 = (p_stats->read_misses_l2 + p_stats->write_misses_l2) /
                 ((double) p_stats->accesses_l2);

    double aat2 = ht2 + mr2 * 500;

    double mp1 = aat2 * (MAX_VC_SIZE == 0 ? 1 : (
            (p_stats->accesses_vc - p_stats->victim_hits) /
            ((double) p_stats->accesses_vc)
    ));
